...
router.get('/us/:userId', checkAuth, checkAuth2, function (req, res, next) {
...
router.patch('/:entryId', function (req, res) {
  ...
  var week = Math.floor((tmp - zero) / (24 * 60 * 60 * 1000) / 7)
  Entry.update({_id: id}, {$set: {duration: req.body.newDuration, length: req.body.newLength, week: week}}).exec().then(result => {
    console.log(result)
